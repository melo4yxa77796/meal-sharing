...
const Meal = ({ meal }) => {
  ...
  const fetchAvailableSpots = async () => {
    try {
      const response = await fetch(`${apiUrl}/api/meals/${meal.id}/spots`);
      if (response.ok) {
        const data = await response.json();
        setAvailableSpots(data.availableSpots);
      } else {
  ...
  const fetchLikes = async () => {
    try {
      const response = await fetch(`${apiUrl}/api/meals/${meal.id}/likes`);
      if (response.ok) {
        const data = await response.json();
        setLikes(data.totalLikes || 0);
      } else {
  ...
  const handleLike = async () => {
    ...
    try {
      const response = await fetch(`${apiUrl}/api/meals/${meal.id}/like`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  return (
    <div className="meal-card">
      ...
      <div className="meal-likes">
        ...
        <span className="likes-count">{likes} ❤️</span>
      </div>
    </div>
  );
};
